...
pub contract AFLMarketplace {
    ...
    pub event AFLMomentListed(id: UInt64, price: UFix64, seller: Address?)
    // emitted when the price of a listed moment has changed
    pub event AFLMomentPriceChanged(id: UInt64, newPrice: UFix64, seller: Address?)
    // emitted when a token is purchased from the market
    pub event AFLMomentPurchased(id: UInt64, price: UFix64, seller: Address?)
    // emitted when a moment has been withdrawn from the sale
    pub event AFLMomentWithdrawn(id: UInt64, owner: Address?)
    ...
    pub resource interface SalePublic {
        ...
        pub fun purchase(tokenID: UInt64, buyTokens: @FungibleToken.Vault): @AFLNFT.NFT {
            post {
    ...
    // holds a AFL Collection resource to store the moments that are for sale.
    ...
    pub resource SaleCollection: SalePublic {
        ...
        access(self) var forSale: @AFLNFT.Collection
        ...
        init (ownerCapability: Capability, beneficiaryCapability: Capability, cutPercentage: UFix64) {
            ...
            self.forSale <- AFLNFT.createEmptyCollection() as! @AFLNFT.Collection
        ...
        pub fun listForSale(token: @AFLNFT.NFT, price: UFix64) {
            ...
            emit AFLMomentListed(id: id, price: price, seller: self.owner?.address)
        }
        ...
        //
        // Returns: @AFL.NFT: The nft that was withdrawn from the sale
        pub fun withdraw(tokenID: UInt64): @AFLNFT.NFT {
            ...
            emit AFLMomentWithdrawn(id: token.id, owner: self.owner?.address)
        ...
        //
        // Returns: @AFL.NFT: the purchased NFT
        pub fun purchase(tokenID: UInt64, buyTokens: @FungibleToken.Vault): @AFLNFT.NFT {
            pre {
            ...
            emit AFLMomentPurchased(id: tokenID, price: price, seller: self.owner?.address)
        ...
        pub fun changePrice(tokenID: UInt64, newPrice: UFix64) {
            ...
            emit AFLMomentPriceChanged(id: tokenID, newPrice: newPrice, seller: self.owner?.address)
        }
